...
function getIndonesianAudioUrl(surahNumber, ayatNumber) {
    ...
    return `https://archive.org/download/AlQuran-terjemahan-indonesia-tanpa-bahasa-arab/${paddedSurah}${paddedAyat}.mp3`;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function displaySurahDetail(surah, tafsir) {
    ...
    surahDetail.innerHTML = `
          <div class="bg-white dark:bg-gray-800 p-2 rounded-lg shadow-md">
            ...
            <input type="checkbox" id="includeIndonesianAudio" class="mr-2">
            <label for="includeIndonesianAudio" class="text-sm text-gray-700 dark:text-gray-300">Indonesian Audio</label>
            </div>
  ...
  function playNextAyat(ayat) {
    ...
    audio.onended = () => {
      ...
      if (includeIndonesianAudio) {
        playIndonesianAudioAutoPlay(currentSurah.nomor, currentAyatIndex + 1);
      } else {
        currentAyatIndex++;
        playNextAyat(ayat);
      }
  ...
  function playIndonesianAudioAutoPlay(surahNumber, ayatNumber) {
    const includeIndonesianAudio = document.getElementById("includeIndonesianAudio").checked;
    if (!includeIndonesianAudio || currentAyatIndex === -1) {
      // Skip Indonesian audio if not checked or for Bismillah
      currentAyatIndex++;
