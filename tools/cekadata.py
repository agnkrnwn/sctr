...
for surah in range(1, 115):
    ...
    for ayat in range(1, expected_ayat + 1):
        ...
        if not os.path.exists(ayat_path):
            missing_ayat.append(ayat)
    
    if missing_ayat:
        incomplete_surahs.append((surah, expected_ayat, expected_ayat - len(missing_ayat), missing_ayat))
...
if incomplete_surahs:
    ...
    for surah, expected, actual, missing in incomplete_surahs:
        print(f"Surah {surah}: Seharusnya {expected} ayat, ditemukan {actual} ayat")
        print(f"Ayat yang tidak ada: {', '.join([f'{surah:03d}{ayat:03d}.mp3' for ayat in missing])}")
else:
