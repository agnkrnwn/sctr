...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.fetchSurahDetail = async function (nomorSurah) {
    try {
      if (surahCache.has(nomorSurah)) {
        const surahData = surahCache.get(nomorSurah);
        displaySurahDetail(surahData);
        surahDetail.scrollIntoView({ behavior: "smooth" });
        return;
      }
  
      const surahResponse = await fetch(`./suratbaru/${nomorSurah}.json`);
      let surahData;
      ...
      if (surahData.code === 200 || surahData.data) {
        ...
        surahCache.set(nomorSurah, currentSurah);
        displaySurahDetail(currentSurah);
        surahDetail.scrollIntoView({ behavior: "smooth" });
      } else {
        let errorMessage = `Gagal memuat detail surah: ${surahData.message || 'Unknown error'}.`;
  ...
  function displaySurahDetail(surah) {
    const qariName = document.getElementById("qariSelect").options[
    ...
    surahDetail.innerHTML = `
      <div class="bg-white dark:bg-gray-800 p-2 rounded-lg shadow-md">
        ...
        <div class="mt-4 space-x-2 flex items-center">
          ...
          <input id="ayatInput" type="number" min="1" max="${surah.jumlahAyat}" class="w-full ml-2 px-3 py-2 border border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 text-sm leading-tight focus:outline-none focus:border-primary-500" placeholder="Go to Ayat">
          ...
          <label class="inline-flex w-full items-center cursor-pointer">
            ...
            <span class="px-4 py-2 bg-primary-500 text-white rounded-md hover:bg-primary-600 transition-colors duration-200 text-sm leading-tight flex items-center justify-center">
    ...
    toggleAyatBtn.addEventListener("click", () =>
      toggleAyat(surah.ayat, surah.ayat.map(a => ({ teks: a.tafsir })), toggleAyatBtn, ayatContainer)
    );
    ...
    displayAyatWithTafsir(surah.ayat, surah.ayat.map(a => ({ teks: a.tafsir })), ayatContainer);
...
function playNextAyat(ayat) {
    ...
    audio.onended = () => {
        ...
        if (includeIndonesianAudio) {
            playIndonesianAudioForAutoPlay(ayat, currentAyatIndex);
        } else {
            currentAyatIndex++;
            playNextAyat(ayat);
        }
...
function playIndonesianAudioForAutoPlay(ayat, ayatIndex) {
    const includeIndonesianAudio = document.getElementById("includeIndonesianAudio").checked;
    if (!includeIndonesianAudio) {
        currentAyatIndex++;
        playNextAyat(ayat);
    ...
    let audioSrc;
    if (ayatIndex === -1 && currentSurah.nomor !== 9) {
        // Play Bismillah audio for Indonesian
        audioSrc = currentSurah.headerBismillahAudio;
    } else if (!ayat || !ayat[ayatIndex]) {
        console.error("Data ayat tidak tersedia");
        currentAyatIndex++;
        playNextAyat(ayat);
        return;
    } else {
        audioSrc = ayat[ayatIndex].audioIndo;
    }

    if (!audioSrc) {
        console.error("URL audio Indonesia tidak tersedia untuk ayat ini");
        currentAyatIndex++;
        playNextAyat(ayat);
        return;
    }
    ...
    audio.onerror = () => {
        console.error(`Error playing Indonesian audio for ${ayatIndex === -1 ? 'Bismillah' : 'Ayat ' + (ayatIndex + 1)}`);
        currentAyatIndex++;
        playNextAyat(ayat);
    };
}
...
function speakTafsir(text, button) {
  ...
  function displayAyatWithTafsir(ayat, tafsir, container) {
    ...
    function playIndonesianAudio(surahNumber, ayatNumber, button) {
        // Pastikan currentSurah dan ayatnya ada
        if (!currentSurah || !currentSurah.ayat || !currentSurah.ayat[ayatNumber - 1]) {
          console.error("Data surah atau ayat tidak tersedia");
          return;
        }
      
        const audioSrc = currentSurah.ayat[ayatNumber - 1].audioIndo;
